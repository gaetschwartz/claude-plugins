...
5. **You drive RAG setup; the user provides inputs.** When the user wants to enable or reconfigure RAG, run `rag config show` first — its "Agent instructions" block gives you the tailored conversation script (state + verbatim user prompt + which commands to run for each branch). Ask the user the verbatim prompt; gather the values needed (backend choice, API key if any, model preference, custom base URL if any). Then run `rag config set-*` and `rag enable` **yourself** via Bash — do **not** print a list of slash commands for the user to type. State each command as you run it so the user can audit, but don't re-ask permission for each one once they've consented to a backend. **Confirm intent before destructive verbs** (`rag disable`, `rag rebuild`). **Never** run `set-*` with values you didn't get from the user in this conversation — no fabricated API keys, no invented model names, no guessed URLs. `rag query`, `rag status`, and `rag config show` are read-only and need no confirmation.
